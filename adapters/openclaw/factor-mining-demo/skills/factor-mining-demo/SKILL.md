...
description: "Use when OpenClaw should demonstrate the direct vt_ Agent API Key Factor Mining flow: create or submit a plugin.py, run a user-scoped backtest, wait for workflow and job results, fetch artifacts, summarize outcomes, or resume a local-agent run through the bundled helper scripts."
---

# Factor Mining Demo

Use this skill for the Factor Mining Demo flow. OpenClaw writes or
...
OpenClaw runs the helper commands from the plugin directory and summarizes the
result for the user. The user does not need to run the command sequence
manually.
...
When OpenClaw is already running and the user wants to add or switch Agent API
...
- Do not call hosted generation endpoints such as `/brainstorm`, `/generate`,
  or `/regenerate`; the adapter generates code inside OpenClaw.
...
1. Confirm setup.
   ...
   - If OpenClaw is already running and the user needs to use a different key, run
     `python3 scripts/factor_setup.py --browser`.
...
2. Determine whether the user is starting from a public task or a custom idea.
   - For a public task flow, read open tasks, show concise choices, and ask the
     user to pick one unless they explicitly ask OpenClaw to choose.
...
workspace so OpenClaw can render them in the reply. Use the first
...
In the final OpenClaw response, embed every available PNG directly with
...
Use resume with waiting after an interrupted OpenClaw session:
